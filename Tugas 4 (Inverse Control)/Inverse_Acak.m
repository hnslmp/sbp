...
%% ANN Input Acak
input = [u(1:2100) u(2:2101) y(4:2103)' y(3:2102)'];
target = u(3:2102);
...
weight_xz_inv = rand(x_size, z_size) - epsilon_init;
weight_zy_inv = rand(z_size, y_size) - epsilon_init;

for i = 1:z_size
    norma(i) = sqrt(sum(weight_xz_inv(:,i).^2));
    weight_xz_inv(:,i) = beta*((weight_xz_inv(:,i))/norma(i));
end

bias_xz_inv = (rand(1, z_size) - epsilon_init) * beta;
bias_zy_inv = rand(1, y_size) - epsilon_init;
...
while stop == 0 && epoch_count <= epoch
    for n = 1:data_count  
        ...
        z_in = bias_xz_inv + xi*weight_xz_inv;
        ...
        %Hidden -> Output
        y_in = bias_zy_inv + z*weight_zy_inv;
        ...
        %Weight Update
        weight_zy_inv = weight_zy_inv - delta_zy;
        weight_xz_inv = weight_xz_inv - delta_xz;
        bias_zy_inv = bias_zy_inv - delta_zy_bias;
        bias_xz_inv = bias_xz_inv - delta_xz_bias;
...
for n = 1:data_count_test  
    ...
    z_in_test = bias_xz_inv + x_test*weight_xz_inv;
    ...
    %Hidden -> Output
    y_in_test = bias_zy_inv + z_test*weight_zy_inv;
...
save('BobotINV.mat',"weight_zy_inv","weight_xz_inv","bias_zy_inv","bias_xz_inv");